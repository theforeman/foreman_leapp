...
const PreupgradeReportsTable = ({ data = {} }) => {
  ...
  const hasData = hasLoadedDataRef.current;

  const searchProps = useMemo(() => {
    const baseProps = getControllerSearchProps('preupgrade_report_entries');
    const autocompleteUrl = reportId
      ? `/api/v2/preupgrade_report_entries/auto_complete_search?preupgrade_report_id=${reportId}`
      : `/api/v2/preupgrade_report_entries/auto_complete_search`;
  ...
  const { inclusionSet, exclusionSet, ...selectAllOptions } = useBulkSelect({
    ...
    metadata: {
      ...
      selectable: pagedFixableEntries.length,
    },
    initialSearchQuery: '',
  });
